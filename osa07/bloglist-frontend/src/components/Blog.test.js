...
describe("<Blog />", () => {
  ...
  test("Blogin näyttävä komponentti renderöi blogin titlen, authorin mutta ei renderöi oletusarvoisesti urlia eikä likejen määrää", () => {
    ...
    */
    ...
    const element = screen.getByText(
      "Nyt olis user extraktio eristetty middlewareksi",
      { exact: false }
    );
    const author = screen.getByText("JK", { exact: false });
  ...
  test("Url ja likejen määrä näytetään kun blogin kaikki tiedot näyttävää nappia on painettu", () => {
    const button = screen.getByText(
      "Nyt olis user extraktio eristetty middlewareksi - JK"
    );
    userEvent.click(button);

    const likes = screen.getByText("likes", { exact: false });
    const url = screen.getByText("longlong-foo-foo-url", { exact: false });
